...
let lastTrainingError: string | null = null;
...
class PythonFaceWorker {
  ...
  async recognizeFrame(frame: string, maxFaces = 50) {
    ...
    return await this.sendRequest<PythonLiveRecognitionResult>(request, 6000);
  }
}
...
/**
 * Queue-aware training trigger.
 * If training is in progress, it sets a 'dirty' flag to re-run after completion.
 */
export async function requestModelRetrain(getEmployees: () => Promise<Employee[]>, onComplete?: (summary: PythonTrainingSummary | null, error?: Error) => Promise<void>) {
  if (isTraining) {
    trainingRequested = true;
    console.log("[python-face] Training already in progress. Queued for next run.");
    return;
  }
  ...
  try {
    console.log("[python-face] Starting automated model retrain...");
    const employees = await getEmployees();
    const summary = await retrainPythonFaceModel(employees);
    lastTrainingError = null;
    if (onComplete) await onComplete(summary);
  } catch (error) {
    console.error("[python-face] Automated training failed:", error);
    lastTrainingError = error instanceof Error ? error.message : "Unknown error";
    if (onComplete) await onComplete(null, error instanceof Error ? error : new Error(String(error)));
  } finally {
    isTraining = false;
    // If another request came in during training, run it now
    if (trainingRequested) {
      console.log("[python-face] Running queued training...");
      setImmediate(() => requestModelRetrain(getEmployees, onComplete));
    }
  }
}

export function getTrainingStatus() {
  return { isTraining, trainingRequested, lastTrainingError };
}
