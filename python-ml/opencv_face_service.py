...
- Multi-scale cascade detection for more robust face detection in varying conditions
- Confidence calibration: normalized and sigmoid-adjusted for realistic percentage displays
- Voting ensemble over multiple frames for gate verification (majority vote + distance averaging)
- More robust distance scaling against threshold for better confidence mapping
- Improved padding and equalization in face crop pipeline
- Support for higher max-faces limits in live recognition
"""
...
def distance_to_confidence(distance: float, threshold: float) -> float:
    ...
    - distance == threshold → confidence ~0.38 (sigmoid midpoint)
    - distance >> threshold → confidence ~0.0

    Uses a sigmoid curve centred at 0.6*threshold so scores just below the
    threshold still appear meaningfully high (e.g. 0.65–0.75).
    """
    if distance is None:
        return 0.0
    # Map distance so that the decision boundary (threshold) lands at ~sigmoid(0)
    # k controls the steepness of the curve
    k = 6.0 / max(threshold, 1.0)
    sigmoid = 1.0 / (1.0 + np.exp(k * (distance - 0.6 * threshold)))
    return clamp(float(sigmoid), 0.0, 1.0)
...
def detect_faces(
    ...
    """
    Multi-scale detection strategy: run cascade at two scale factors and
    merge duplicate detections (keep the largest area per cluster).
    """
    all_faces: list[tuple[int, int, int, int]] = []

    for sf in (args.scale_factor, min(args.scale_factor + 0.05, 1.3)):
        for mn in (args.min_neighbors, max(2, args.min_neighbors - 2)):
    ...
    merged = _nms_faces(all_faces, iou_threshold=0.35)

    return sorted(merged, key=lambda f: f[2] * f[3], reverse=True)
...
def _nms_faces(
    ...
    """Simple greedy NMS keeping the largest box from each overlapping cluster."""
    if not faces:
        return []
    ...
    for i, face in enumerate(sorted_faces):
        ...
        for j in range(i + 1, len(sorted_faces)):
            if not suppressed[j] and _iou(face, sorted_faces[j]) > iou_threshold:
                suppressed[j] = True
...
def prepare_face_crop(
    ...
    # Use CLAHE (Contrast Limited Adaptive Histogram Equalization) instead of
    # plain equalizeHist for better performance under uneven lighting
...
def build_response(
    ...
    best_prediction = min(chosen_predictions, key=lambda p: p.distance or float("inf"))
    # Use the best distance (lowest) rather than average for confidence -- more responsive
    best_distance = min(p.distance for p in chosen_predictions if p.distance is not None)
    match_confidence = distance_to_confidence(best_distance, distance_threshold)
    ...
    return {
        ...
        "bestDistance": round_float(best_distance, 3),
        "distanceThreshold": distance_threshold,
        "movementDirection": direction,
        "movementAxis": axis,
        "movementConfidence": direction_confidence,
...
def handle_recognize_frame(
    ...
    for face_box in detected_faces[: max(1, min(max_faces, 50))]:
        ...
        confidence = distance_to_confidence(distance, distance_threshold) if distance is not None else 0.0
...
def main() -> int:
    ...
    print(f"[face-worker] ready threshold={distance_threshold} image_size={image_size}", file=sys.stderr)
