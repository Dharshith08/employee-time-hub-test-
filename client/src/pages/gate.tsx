                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2 text-[11px]">
                    ...
                    <div className="col-span-2 rounded-xl bg-white/80 p-3">
                      <div className="flex items-center justify-between">
                        <p className="uppercase tracking-[0.18em] text-muted-foreground">Match Confidence</p>
                        <p className={cn(
                          "font-mono font-semibold",
                          !lastResult ? "text-muted-foreground"
                            : (lastResult.matchConfidence ?? 0) >= 0.75 ? "text-emerald-700"
                            : (lastResult.matchConfidence ?? 0) >= 0.5 ? "text-amber-600"
                            : "text-rose-600",
                        )}>{formatPercent(lastResult?.matchConfidence)}</p>
                      </div>
                      <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-slate-200">
                        <div
                          className={cn(
                            ...
                            !lastResult ? "w-0"
                              : (lastResult.matchConfidence ?? 0) >= 0.75 ? "bg-emerald-500"
                              : (lastResult.matchConfidence ?? 0) >= 0.5 ? "bg-amber-400"
                              : "bg-rose-500",
                          )}
                          style={{ width: `${Math.round((lastResult?.matchConfidence ?? 0) * 100)}%` }}
                        />
                      </div>
                    </div>
                    <div className="rounded-xl bg-white/80 p-3">
                    ...
                    <div className="rounded-xl bg-white/80 p-3">
                      <p className="uppercase tracking-[0.18em] text-muted-foreground">LBPH Distance</p>
                      <p className="mt-1 font-mono text-foreground">
                        {(lastResult as any)?.bestDistance != null ? `${(lastResult as any).bestDistance}` : "--"}
                      </p>
                    </div>
                    <div className="rounded-xl bg-white/80 p-3">
                ...
                <div className="rounded-2xl border border-border/60 bg-white/80 p-3">
                  ...
                  <div className="relative overflow-hidden rounded-[1.5rem] border border-slate-200 bg-slate-950">
                    ...
                    {detectedFaceBoxStyle && (
                      <div
                        className={cn(
                          "absolute rounded-[1.25rem] border-[3px] shadow-[0_0_24px_rgba(0,0,0,0.25)]",
                          !lastResult ? "border-cyan-300"
                            : (lastResult.matchConfidence ?? 0) >= 0.75
                              ? "border-emerald-400 shadow-[0_0_0_1px_rgba(52,211,153,0.5),0_0_24px_rgba(16,185,129,0.25)]"
                              : (lastResult.matchConfidence ?? 0) >= 0.5
                                ? "border-amber-400 shadow-[0_0_0_1px_rgba(251,191,36,0.5),0_0_24px_rgba(245,158,11,0.22)]"
                                : "border-rose-400 shadow-[0_0_0_1px_rgba(248,113,113,0.5),0_0_24px_rgba(239,68,68,0.22)]",
                        )}
                      ...
                      >
                        <div className="absolute left-0 top-0 -translate-y-[calc(100%+0.45rem)] rounded-full bg-black/80 px-3 py-1 text-[11px] font-semibold text-white shadow-lg">
                          ...
                          {lastResult.matchConfidence != null && (
                            <span className={cn(
                              "ml-2 text-[10px]",
                              (lastResult.matchConfidence ?? 0) >= 0.75 ? "text-emerald-300"
                                : (lastResult.matchConfidence ?? 0) >= 0.5 ? "text-amber-300"
                                : "text-rose-300",
                            )}>{formatPercent(lastResult.matchConfidence)}</span>
                          )}
                        </div>
                      </div>
                    )}
