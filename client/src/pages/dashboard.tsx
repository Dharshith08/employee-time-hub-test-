...
function OverviewCard(props: {
  ...
  return (
    <Card className="glass-card premium-border overflow-hidden group hover:translate-y-[-2px] transition-all duration-300">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
        <CardTitle className="text-sm font-medium opacity-80">{props.title}</CardTitle>
        <div className={`rounded-lg p-2 ${props.accent?.replace('text-', 'bg-') || 'bg-muted'} bg-opacity-10 group-hover:scale-110 transition-transform`}>
      ...
      <CardContent>
        {props.isLoading ? (
          ...
          <div className={`text-3xl font-bold tracking-tight ${props.accent ?? "text-foreground"}`}>{props.value}</div>
        )}
        <p className="mt-2 text-xs text-muted-foreground/70">{props.hint}</p>
      </CardContent>
      <div className="absolute bottom-0 left-0 h-1 w-full opacity-20 bg-gradient-to-r from-transparent via-current to-transparent" />
    </Card>
  );
}

function TrainingStatusIndicator() {
  const [status, setStatus] = useState<{ isTraining: boolean; lastTrainingError: string | null } | null>(null);

  // Poll for training status every 3 seconds
  useState(() => {
    const fetchStatus = async () => {
      try {
        const res = await fetch("/api/stats/training");
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/10 border border-primary/20 animate-pulse transition-all">
      <RefreshCw className="size-3 text-primary animate-spin" />
      <span className="text-[10px] uppercase tracking-[0.15em] font-bold text-primary">Model Training in Progress</span>
    </div>
  );
}
  ...
  return (
    <div className="space-y-8 p-6 md:p-8 animate-in fade-in duration-500">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-4">
            <h1 className="text-3xl font-bold text-foreground text-gradient">Operations Dashboard</h1>
          ...
          {lastUpdatedAt > 0 && (
            <p className="mt-2 text-[10px] uppercase tracking-wider text-muted-foreground/60 flex items-center gap-2">
              <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse" />
              Live refresh: {format(new Date(lastUpdatedAt), "hh:mm:ss a")}
            </p>
          )}
        </div>
      ...
      <Card className="glass-card premium-border overflow-hidden">
        <CardHeader className="pb-4 bg-muted/5 transition-colors">
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-4">
        {/* These cards will also be updated via the OverviewCard component automatically */}
      </div>
      ...
      <Card className="glass-card premium-border overflow-hidden">
        <CardHeader className="bg-muted/5 transition-colors">
